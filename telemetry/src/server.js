    ...
    if (role === 'driver' || role === 'repartidor') {
      ...
      socket.on('location:update', async (data) => {
        ...
        try {
          ...
          // C. Almacenamiento Transaccional en GEOSET y Key de Expiración
          await pubClient.geoAdd(geoKey, {
            longitude: lonNum,
            latitude: latNum,
            member: String(userId)
          ...
          await pubClient.hSet(telemetryMetaKey, {
            latitude: String(latNum),
            longitude: String(lonNum),
            heading: headNum !== null ? String(headNum) : '',
            timestamp: String(Date.now())
          });
          await pubClient.expire(telemetryMetaKey, 3600);

          // D. Retransmisión en tiempo real a salas de monitoreo
          const payload = {
            driverId: userId,
            latitude: latNum,
            longitude: lonNum,
          ...
          trackingNamespace.emit('location:broadcast', payload);
          trackingNamespace.to(`monitor:driver:${userId}`).emit('location:driver', payload);

          if (orderId) {
            trackingNamespace.to(`order:${orderId}`).emit('location:order', payload);

            const destKey = `order:destination:${orderId}`;
            const dest = await pubClient.hGetAll(destKey);

            if (dest && dest.latitude && dest.longitude) {
              const destLat = parseFloat(dest.latitude);
              const destLon = parseFloat(dest.longitude);
              const distanceToDestMeters = getHaversineDistance(latNum, lonNum, destLat, destLon);

              trackingNamespace.to(`order:${orderId}`).emit('location:eta_update', {
                orderId,
                distanceMeters: Math.round(distanceToDestMeters),
                estimatedMinutes: Math.max(1, Math.round((distanceToDestMeters / 1000) / 20 * 60))
              });

              if (distanceToDestMeters <= 50) {
                const arrivalNoticeKey = `order:arrival_notified:${orderId}`;
                const alreadyNotified = await pubClient.get(arrivalNoticeKey);

                if (!alreadyNotified) {
                  await pubClient.set(arrivalNoticeKey, '1', { EX: 1800 });

                  console.log(`[GEOFENCE] Driver ${userId} arrived at destination for order ${orderId} (${Math.round(distanceToDestMeters)}m)`);

                  trackingNamespace.to(`order:${orderId}`).emit('order:driver_arrived', {
                    orderId,
                    driverId: userId,
                    distanceMeters: Math.round(distanceToDestMeters),
                    timestamp: Date.now()
                  ...
                  await logTelemetrySecurityEvent(
                    ...
                    'LOW',
                    {
                      reason: 'Conductor dentro del radio de 50 metros del destino',
                      orderId,
                      distanceMeters: Math.round(distanceToDestMeters)
                    },
                    'order',
          ...
          console.error(`[ERROR] Location update processing failed for driver ${userId}:`, err);
        }
      });

      socket.on('disconnect', () => {
        console.log(`[INFO] Driver disconnected: ${socket.id} (User: ${userId})`);
      });
    }

    if (role === 'admin' || role === 'customer' || role === 'comercio') {
      socket.on('subscribe:driver', (data) => {
        const { driverId } = data;
        if (driverId) {
          socket.join(`monitor:driver:${driverId}`);
          console.log(`[INFO] User ${userId} subscribed to driver:${driverId}`);
        }
      });
      ...
      socket.on('subscribe:order', (data) => {
        ...
        if (orderId) {
          socket.join(`order:${orderId}`);
          console.log(`[INFO] User ${userId} subscribed to order:${orderId}`);
        }
      });
