...
export default function SystemMaintenancePage() {
  ...
  useEffect(() => {
    if (!token || !autoRefreshLogs) return;
    const logsInterval = setInterval(fetchLogs, 4000);
    return () => clearInterval(logsInterval);
  }, [token, autoRefreshLogs]);
  ...
  const handleEnableMaintenance = async () => {
    ...
    try {
      ...
      if (isNaN(duration) || duration <= 0) {
        throw new Error('La duracion debe ser un numero de minutos valido mayor a cero.');
      }

      const res = await axios.post(
        `${API_URL}/api/manage/system/maintenance/enable`,
        { message: maintenanceMessage, durationMinutes: duration },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handlePanicRevocation = async () => {
    ...
    try {
      ...
      // Tras presionar botón de pánico, el token propio del admin quedará revocado en el siguiente request.
      // El backend responderá con 440 en la próxima operación y lo deslogueará.
      setTimeout(() => {
    ...
    <HudContainer>
      ...
      <PanelCard className="margin-top">
        <PanelTitle>
          ...
          <LogControls>
            <LogControlBtn onClick={fetchLogs}>Refrescar Logs</LogControlBtn>
...
const RetroTextInput = styled.input`
  ...
  &:focus {
    outline: none;
    border-color: #00ff00;
    box-shadow: 0 0 10px rgba(0, 255, 0, 0.2), inset 0 2px 4px rgba(0, 0, 0, 0.8);
  }
...
const ArcadeButton = styled.button<{ $color: 'amber' | 'green' | 'red' }>`
  width: 60px;
  height: 60px;
  border-radius: 50%;
