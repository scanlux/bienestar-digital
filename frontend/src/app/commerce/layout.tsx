  ...
  return (
    <LayoutContainer>
      <Sidebar>
        <SidebarHeader>
          <Cube>C</Cube>
          <LogoText>Commerce<span>Core</span></LogoText>
        </SidebarHeader>

        <UserInfo>
          <UserName>{user?.nombre || 'Administrador'}</UserName>
          <UserRole>{user?.rol || 'Rol: comercio'}</UserRole>
        </UserInfo>
        ...
        <NavList>
          {(() => {
            ...
            return getNavItems().map((item) => {
              const isActive = pathname === item.path || (item.path !== '/commerce/dashboard' && pathname.startsWith(item.path));
              return (
      ...
      <MainContent>
        ...
        <ContentWrapper id="commerce-scroll-container">
          {children}
        </ContentWrapper>
      </MainContent>
    </LayoutContainer>
...
const NavLink = styled(Link)<{ $active?: boolean }>`
  ...
  &:hover {
    background: ${props => props.$active ? 'rgba(16, 185, 129, 0.15)' : 'rgba(255, 255, 255, 0.03)'};
    color: #fff;
    ${Icon} { opacity: 1; }
  }
`;
