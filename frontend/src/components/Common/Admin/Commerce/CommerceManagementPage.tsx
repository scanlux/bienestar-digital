...
import { PageContainer, CommercesGrid, EmptyState, SearchInput, SearchIconIcon, SedeGhostCard } from './CommerceStyles';
...
export default function CommerceManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error(e);
      toast.error(e.response?.data?.message || e.response?.data?.error || 'Error al guardar los cambios del comercio');
    }
  };
  ...
  const handleEditSedeFromModal = (sede: any) => {
    router.push(`/admin/dashboard/stores/${sede.id}/profile`);
  };
