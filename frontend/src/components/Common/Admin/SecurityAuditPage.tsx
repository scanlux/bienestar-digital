...
export default function SecurityAuditPage() {
  ...
  const [activeTab, setActiveTab] = useState<'audit' | 'operators'>('audit');
  const [users, setUsers] = useState<any[]>([]);
  const [usersLoading, setUsersLoading] = useState(false);
  const [resetPinUrl, setResetPinUrl] = useState<string | null>(null);
  const [resetPinEmail, setResetPinEmail] = useState<string>('');
  
  const limit = 15;
  const canManageRbac = user?.permissions?.includes('manage_rbac');
  ...
  useEffect(() => {
    if (token && activeTab === 'operators') {
      fetchUsers();
    }
  }, [token, activeTab]);

  const handleUnlockPin = async (userId: number, email: string) => {
    showConfirm({
      title: 'Desbloquear PIN Financiero',
      message: `¿Estás seguro de que deseas desbloquear el PIN financiero para el usuario ${email}? Los intentos fallidos se restablecerán a 0.`,
      onConfirm: async () => {
        try {
          const res = await axios.post(`${API_URL}/api/manage/users/${userId}/unlock-pin`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
          toast.success(res.data.message || 'PIN desbloqueado con éxito.');
          fetchUsers();
        } catch (error: any) {
          toast.error(error.response?.data?.error || 'Error al desbloquear el PIN.');
        }
      }
    });
  };

  const handleGenerateResetLink = async (userId: number, email: string) => {
    try {
      const res = await axios.post(`${API_URL}/api/manage/users/${userId}/reset-pin-link`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setResetPinUrl(res.data.resetUrl);
      setResetPinEmail(email);
      toast.success('Enlace de recuperación generado con éxito.');
    } catch (error: any) {
      toast.error(error.response?.data?.error || 'Error al generar el enlace.');
    }
  };
    ...
    <Container>
      ...
      {canManageRbac && (
        <TabsContainer>
          ...
          <Tab $active={activeTab === 'operators'} onClick={() => setActiveTab('operators')}>
            Desbloqueos de PIN Financiero
          </Tab>
        </TabsContainer>
      )}
      ...
      {activeTab === 'operators' && (
        <>
          {usersLoading ? (
            <LoadingState>
              <Spinner />
              <p>Obteniendo usuarios del sistema...</p>
            </LoadingState>
          ) : users.length === 0 ? (
            <EmptyState>
              <p>No se encontraron usuarios administradores o de sistema.</p>
            </EmptyState>
          ) : (
            <TableContainer>
              <Table>
                  ...
                  {users.map(u => (
                    <tr key={u.id}>
                      <td>
                        ...
                        <UserDetail>ID: {u.id} | {u.nombre}</UserDetail>
                      </td>
                      <td>
                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                          <SeverityBadge $background="rgba(255, 255, 255, 0.1)" style={{ textShadow: 'none' }}>
                            {u.rol?.toUpperCase()}
                          </SeverityBadge>
                          {u.roles?.map((r: string) => (
                            <SeverityBadge key={r} $background="rgba(16, 185, 129, 0.2)" style={{ color: '#10b981', textShadow: 'none' }}>
                              {r}
                            </SeverityBadge>
                          ))}
                        </div>
                      ...
                      <td style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>
                        {u.financial_pin_attempts} / 3
                      </td>
                      <td>
                        {u.financial_pin_locked === 1 ? (
                          <SeverityBadge $background="linear-gradient(135deg, #ff3b30 0%, #ff2d55 100%)">
                            BLOQUEADO
                          </SeverityBadge>
                        ) : (
                          <SeverityBadge $background="linear-gradient(135deg, #34c759 0%, #00c7be 100%)">
                            ACTIVO
                          </SeverityBadge>
                        )}
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                          {u.financial_pin_locked === 1 && (
                            <Button onClick={() => handleUnlockPin(u.id, u.email)}>
                              Desbloquear PIN
                            </Button>
                          )}
                          <Button onClick={() => handleGenerateResetLink(u.id, u.email)} secondary>
                            Restablecer PIN
                          </Button>
                        </div>
                      </td>
      ...
      {/* Reset PIN Link Modal Portal */}
      {resetPinUrl && typeof window !== 'undefined' && document.getElementById('modal-portal-root') && 
        createPortal(
          <ModalOverlay onClick={() => setResetPinUrl(null)}>
            <ModalContent onClick={e => e.stopPropagation()}>
              <ModalHeader>
                <div>
                  <ModalTitle>Enlace de Recuperación Generado</ModalTitle>
                  <ModalSubtitle>Para: {resetPinEmail}</ModalSubtitle>
                </div>
                <CloseButton onClick={() => setResetPinUrl(null)}>&times;</CloseButton>
              </ModalHeader>
              <ModalBody style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)', margin: 0 }}>
                  El correo con las instrucciones ha sido enviado. También puedes copiar el enlace directo a continuación para compartirlo de forma manual con el operador:
                </p>
                <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px', borderRadius: '8px', wordBreak: 'break-all', fontFamily: 'monospace', fontSize: '0.85rem', color: '#10b981' }}>
                  {resetPinUrl}
                </div>
                <Button onClick={() => {
                  navigator.clipboard.writeText(resetPinUrl);
                  toast.success('Enlace copiado al portapapeles.');
                }} style={{ alignSelf: 'flex-start' }}>Copiar Enlace</Button>
              </ModalBody>
              <ModalFooter>
                <Button onClick={() => setResetPinUrl(null)}>Cerrar</Button>
              </ModalFooter>
            </ModalContent>
          </ModalOverlay>,
          document.getElementById('modal-portal-root')!
...
const Tab = styled.button<{ $active: boolean }>`
  background: none;
  border: none;
  color: ${props => props.$active ? '#10b981' : 'rgba(255, 255, 255, 0.5)'};
  ...
  &:hover {
    color: ${props => props.$active ? '#10b981' : '#fff'};
  }
`;
