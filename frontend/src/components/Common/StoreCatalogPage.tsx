...
const ProductSelectionCard = styled.div<{ $isEnabled: boolean }>`
  ...
  &:hover {
    ...
    border-color: ${p => p.$isEnabled ? 'rgba(16, 185, 129, 0.3)' : 'rgba(255, 255, 255, 0.1)'};
  }
...
export default function StoreCatalogPage({ params }: { params: { storeId: string, categoriaId: string } }) {
  ...
  const handleBackClick = () => {
    if (isAdminPath) {
      router.push(`/admin/dashboard/stores/${params.storeId}`);
    } else {
