...
async function migrate() {
  console.log('=== Iniciando Migración v10: Simplificación del Flujo de Repartidores ===');
  const sqlFilePath = path.join(__dirname, 'v10_simplify_drivers.sql');
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        // Ignorar errores comunes si las columnas o tablas ya existen o no existen
        ...
        const isIgnorable = [
          ...
          'ER_CANT_DROP_FIELD_OR_KEY', // Columna o FK a eliminar no existe
          '1060',                      // Duplicated column
          '1061',                      // Duplicate key name
          '1050',                      // Table already exists
          '1091',                      // Can't drop field/key; check that it exists
          '1022',                       // Can't write; duplicate key in table
          'ER_BAD_TABLE_ERROR'         // Table doesn't exist
    ...
    await conn.commit();
    console.log('=== Migración v10 completada con éxito en la base de datos ===');
  } catch (error) {
    await conn.rollback();
    console.error('=== Migración v10 ABORTADA debido a un error crítico ===');
    console.error(error);
    process.exit(1);
  } finally {
    conn.release();
