...
async function migrate() {
  console.log('=== Iniciando Migración v9: DOMI Tether Model ===');
  const sqlFilePath = path.join(__dirname, 'v9_domi_tether.sql');
  ...
  try {
    ...
    await conn.commit();
    console.log('=== Migración v9 completada con éxito en la base de datos ===');
  } catch (error) {
    await conn.rollback();
    console.error('=== Migración v9 ABORTADA debido a un error crítico ===');
    console.error(error);
    process.exit(1);
  } finally {
    conn.release();
