-- =========================================================
-- Rediseño de Identidades y Cuentas - Reset Completo DDL (Rev. 8)
-- =========================================================
...
-- 2. Nuevas Tablas de Sistema y Operadores
...
CREATE TABLE `store_operators` (
  ...
  `estado` enum('activo','inactivo') DEFAULT 'activo',
  `permiso_pedidos` tinyint(1) NOT NULL DEFAULT 0,
  `permiso_catalogo` tinyint(1) NOT NULL DEFAULT 0,
  `permiso_videos` tinyint(1) NOT NULL DEFAULT 0,
  `permiso_horarios` tinyint(1) NOT NULL DEFAULT 0,
  `permiso_cuentas` tinyint(1) NOT NULL DEFAULT 0,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `email` (`email`),
...
CREATE TABLE `delivery_companies` (
  ...
  UNIQUE KEY `nit` (`nit`),
  CONSTRAINT `fk_delivery_company_user` FOREIGN KEY (`usuario_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Agregar restricción en profiles para delivery_companies
ALTER TABLE `profiles` ADD CONSTRAINT `fk_profile_delivery_company` FOREIGN KEY (`delivery_company_id`) REFERENCES `delivery_companies` (`id`) ON DELETE SET NULL;

-- 3. Catálogo y Negocios
...
CREATE TABLE `commerces` (
  ...
  UNIQUE KEY `usuario_id` (`usuario_id`),
  CONSTRAINT `fk_commerce_user` FOREIGN KEY (`usuario_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `stores` (
  ...
  CONSTRAINT `fk_store_user` FOREIGN KEY (`usuario_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT,
...
-- 5. Privilegios de Sedes (Pivotes - Removidos en v11)

-- 6. Horarios y Cuentas Bancarias
...
-- 7. Intermedias de Usuarios y Sedes
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Billeteras de usuarios, tiendas y sistema.';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Libro mayor inmutable. NUNCA se hacen UPDATE ni DELETE.';

-- 9. Transaccional / Operaciones
