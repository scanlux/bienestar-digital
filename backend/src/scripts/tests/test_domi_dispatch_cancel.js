...
async function runTest() {
  ...
  console.log('=== INICIANDO PRUEBA DE INTEGRACION: CANCELACION EN listo_despacho DOMI ===');
  ...
  try {
    ...
    // Wallet del cliente: 50 DOMIs exactos (suficiente para cubrir la orden de 25,000 COP / 400 = 62.5 DOMIs? No, 25,000 / 400 = 62.5, por lo tanto 50 DOMIs es insuficiente.
    // Pongamos 100 DOMIs!)
    ...
    console.log('Pedido insertado en DB:', {
      ...
      driver_domi_cost: orderCreated.driver_domi_cost, // 9.75 DOMI ($3900 COP / 400)
      driver_cost_domi_snapshot: orderCreated.driver_cost_domi_snapshot // 0.75 DOMI ($300 COP / 400)
    });
    ...
    console.log('\n--- PASO 6: Cliente cancela el pedido en estado listo_despacho (DOMI) ---');
    await orderService.updateOrderStatus(
      { id: testClientUserId, rol: 'customer', actorType: 'user' },
      orderId,
      'cancelado',
      { body: { observation: 'Me arrepentí de comprar con DOMI' } }
    );

    // 7. VERIFICAR IMPACTOS
    console.log('\n--- PASO 7: Validando impacto del reembolso de cancelación DOMI ---');
    ...
    console.log('Balances tras cancelación DOMI:');
    console.log(' - Cliente balance_custody:', clientBalAfterCancel.balance_custody);
    console.log(' - Cliente locked_balance (esperado 0.0):', clientBalAfterCancel.locked_balance);
    console.log(' - Cliente Score (esperado 58, bajó de 60 por -2 puntos):', clientUser.domi_score);
    ...
    if (clientUser.domi_score !== 58) {
      throw new Error(`Error: Score de cliente esperado 58, obtenido ${clientUser.domi_score}`);
    }

    // El cliente inicialmente pagó 62.5 DOMIs (bloqueados).
    // Domicilio = 9.75 DOMI. Compensación conductor = 9.75 * 0.5 = 4.875 DOMI.
    // Devolución al cliente = 9.75 - 4.875 = 4.875 DOMI.
    // Custody inicial cliente = 100.0.
    // Al crear se descontó 62.5 (quedó 37.5).
    // Al cancelar recibe reembolso de 4.875. Custody final cliente = 37.5 + 4.875 = 42.375 DOMI.
    const expectedClientCustody = 42.375;
    ...
    // Al cancelar recibe 4.875 DOMI de compensación de domicilio.
    // Conductor custody final = 9.25 + 4.875 = 14.125 DOMI.
    const expectedDriverCustody = 14.125;
    ...
    // Productos = 62.5 - 9.75 = 52.75 DOMI.
    // Sede recibe productos * 99.6% = 52.75 * 0.996 = 52.539 DOMI.
    // Sede custody final = 9.0 + 52.539 = 61.539 DOMI.
    const expectedStoreCustody = 61.539;
    ...
    // Recibe la tasa unificada de procesamiento = 52.75 * 0.004 = 0.211 DOMI.
    // Sistema custody final = 10.0 + 0.211 = 10.211 DOMI.
    const expectedSysCustody = 10.211;
    ...
    console.log('\n=== TEST DE CANCELACIÓN DOMI EN listo_despacho EXITOSO ===');
  } catch (error) {
    console.error('Error durante la prueba de integración listo_despacho DOMI:', error);
    process.exit(1);
  } finally {
